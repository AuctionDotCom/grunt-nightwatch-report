...
var jadeTemplateIndexHtml = path.join(__dirname, '..', 'templates', 'output.jade');
...
function writeSummaryJsonToFile(outputDir, summaryJsonString, grunt) {
    ...
    var summaryDir = outputDir + 'summary';
...
function writeTemplatedSummaryReports(summary, outputDir, grunt) {
    ...
    var indexFn = jade.compileFile(jadeTemplateIndexHtml, jadeIndexOptions);
    ...
    grunt.registerMultiTask('nightwatch_report', 'Aggregate and process Nightwatch.js test suite reports to create easy to digest HTML reports.', function() {
        ...
        (summary.suites || []).forEach(function(suite) {
            (suite.cases || []).forEach(function(testcase) {
                ...
                var linkIndex = 1;
                if(name)
                    testcase.link = options.saucelabsData[name] && options.saucelabsData[name][linkIndex] || undefined;
            });
        });
